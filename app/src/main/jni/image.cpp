...
void native_blur(JNIEnv *env, jobject* object, jobject srcBitmap,jint radius, jint iterations,jint startRow,jint endRow) {
	void *buff1;
	void *buff2;
	...
	if (startRow >= 0 && endRow < info.height && startRow < endRow) {
		h = endRow - startRow+1;
		buff1=(int*)(buff1)+(startRow*info.width);
	} else {
		h = info.height;
	}
	buff2=new int[info.width*h];
	Boxblur2((int*) buff1, (int*) buff2, info.width, h,radius,iterations);
